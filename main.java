...
public class main {
    public static void main(String[] args){
        ...
        //Multiply_Two_Strings multiply_two_strings = new Multiply_Two_Strings();
        //Integer_Panindrome integer_panindrome = new Integer_Panindrome();
        //median_of_two_Sorted_arrays median_of_two_sorted_arrays = new median_of_two_Sorted_arrays();
        //divide_without_division divide_without_division = new divide_without_division();
        //perfectSquare perfectSquare = new perfectSquare();
        //Sum_of_squares Sumofsquare = new Sum_of_squares();
        //length_of_last_word length_of_last_word = new length_of_last_word();
        //rotateLinkedList rotateLinkedList = new rotateLinkedList();
    }
}
//Hacker Rank problem Correct but timeout exceeded

/*
        List<Integer> Temp = new ArrayList<>();
        for(int i = 0; i < ranked.size(); i++){
            if(!Temp.contains(ranked.get(i))){
                Temp.add(ranked.get(i));
            }
        }
        ...
        for(int i = 0; i < player.size(); i++){
            ...
            Sol.add(Temp.indexOf(player.get(i)) + 1);
            Temp.remove(player.get(i));

        }
        return Sol;

 */
//Find all combinations in a range
class Combinations_In_Range{
...
//Medium problem, beats 100% of other submissions
class rotateLinkedList{
    rotateLinkedList(){

        ListNode head = GenerateList(12678932);
        int k = 82374;
        if(head == null || k == 0) System.out.println(head);
    ...
    ListNode GenerateList(int num){
        int[] Vals = String.valueOf(num).chars().map(c -> c-'0').toArray();
...
//Accepted Solution faster than 50%
class length_of_last_word{
    length_of_last_word(){
        String s = " ";
        if(s.isEmpty() || s.isBlank())  System.out.println(0);
        else {
...
class Sum_of_squares{
    Sum_of_squares(){
        ...
        for(int i = 0; i <= c; i++){
            double val1 = Math.pow(i, 2);
            if(val1 > c){
                tf = false;
                break;
            }
            double Square = Math.sqrt((c - val1));

            if(Square - Math.round(Square) == 0){
                tf = true;
                break;
            }
        }
...
class perfectSquare{
    perfectSquare(){
        ...
        while(true){
            long Temp = ((long) current) * ((long) current);
...
//This solution was accepted by leetcode. O(n + m) runtime specification.
//Difficulty hard
class median_of_two_Sorted_arrays{
    median_of_two_Sorted_arrays() {
        ...
        if(nums1.length != 0)
            System.arraycopy(nums1, 0, sol, 0, nums1.length);
        if(nums2.length != 0)
            System.arraycopy(nums2, 0, sol, nums1.length, nums2.length);
        ...
        else if(sol.length % 2 == 0){
            ans = ((double)sol[sol.length/2] + (double)sol[(sol.length/2)-1]) / 2;
        }
        else{
            ans = sol[(sol.length / 2)];
        }

        System.out.println(ans);
    }
}
//Checking for integer palindromes
class Integer_Panindrome{
    Integer_Panindrome(){
        int x = 1111;
        String Sol = "";
        if(x < 0){
            System.out.println("False boiii!");
        }
        else{
            int[] list = String.valueOf(x).chars().map(c -> c-'0').toArray();
            if(list.length == 1){
                Sol = "True";
            }
...
//In this case, I decided to build the algorithm that we learned as children, where you do long multiplication.
/*
  5
  56
x  9
= 504
 */ //Almost Working solution but off sometimes.  Internal Bug?
class Multiply_Two_Strings {
    Multiply_Two_Strings(){
        String num1 = "123654";
        String num2 = "654123";

        String[] Partials = new String[Math.min(num1.length(), num2.length())];
        //Build Leading 0's
        for(int i = 0; i < Partials.length; i++){
            Partials[i] = "";
            for(int j = 0; j < i; j++){
                Partials[i] = "0" + Partials[i];
            }
        }

        //Deciding which string is shorter
        String minStr = num1.length() < num2.length() ? num1 : num2;
        String maxStr = num1 == minStr ? num2 : num1;

        String sol = "";

        int Carry = 0; int Val = 0; int counter = 0;
        //Because we are multiplying a single digit with another single digit, the largest number possible is 9*9 = 81
        for(int i = Partials.length - 1; i >= 0; i--){
            for(int j = maxStr.length() - 1; j >= 0; j--){
                int f = Character.getNumericValue(minStr.charAt(i)); int s = Character.getNumericValue(maxStr.charAt(j));
                Val = f * s + Carry;
                Carry = 0;

                if(Val >= 10){
                    int[] ArrVal = String.valueOf(Val).chars().map(c -> c-'0').toArray();
                    Carry = ArrVal[0];
                    Partials[counter] = ArrVal[1] + Partials[counter];
                }
                else{
                    Partials[counter] = Val + Partials[counter];
                }
            }
            if(Carry != 0) {
                Partials[counter] = Carry + Partials[counter];
                Carry = 0;
            }
            counter++;
        }

        //Reusing predefined variable maxStr
        maxStr = Partials[0];

        for(int i = 0; i < Partials.length; i++){
            maxStr = maxStr.length() < Partials[i].length() ? Partials[i] : maxStr;
        }
        //Reusing predefined variable Val, Carry, and Counter
        Val = 0; Carry = 0; counter = 0;
        for(int i = 1; i <= maxStr.length(); i++){
            for(int j = 0; j < Partials.length; j++){
                counter = Partials[j].length() - i;
                if(counter >= 0){
                    Val = Val + Character.getNumericValue(Partials[j].charAt(counter)) + Carry;
                    Carry = 0;
                }
            }
            if(Val >= 10 && Val < 100){
                int[] valArr = String.valueOf(Val).chars().map(c ->c-'0').toArray();
                Carry = valArr[0];
                sol = valArr[1] + sol;
            }
            else if(Val >= 100){
                int[] valArr = String.valueOf(Val).chars().map(c -> c-'0').toArray();
                String subString = valArr[0] + valArr[1]+ "";
                Carry = Integer.parseInt(subString);
                sol  = valArr[2] + sol;
            }
            else{
                sol = Val + sol;
 ...
 */
//This solution was accepted by leetcode. Runtime: 9ms memory usage: 39.8
class Add_two_numbers{
    Add_two_numbers(){
        ListNode l1 = GenerateList(9999999);
        ListNode l2 = GenerateList(9999);
        ...
        while(l1 != null || l2 != null){
            //First case, both lists do not equal null, meaning both have a number to add together
            if(l1 != null && l2 != null){
                val = l1.val + l2.val + carry;
            }
            //Second case where list 1 is now empty but list 2 still has content, as verified by the while loop
            else if(l1 == null){
                val = l2.val + carry;
            }
            //Third case where list 2 is now empty but list 1 still has content, as verified by the while loop. This could be an else but left as else if for clarity
            else if(l2 == null){
                val = l1.val + carry;
            }
            carry = 0;
            //This statement is true for all cases, if val is greater than 10, there ill always be a carry.
            if(val >= 10){
                int[] Split = String.valueOf(val).chars().map(c -> c-'0').toArray();
                node.val = Split[1];
                carry = Split[0];
            }
            else{
            ...
            //IF there still exists numbers in one of the two lists, create new node and traverse.
            if(l1 != null || l2 != null) {
                node.next = new ListNode();
                node = node.next;
            }
        }
        //Finally, if there is a carry left over. Add it to the end of the list.
        if(carry != 0){
    ...
    private ListNode GenerateList(int input){
        int[] in = String.valueOf(input).chars().map(c -> c-'0').toArray(); //Very important line converts int to int array
        ListNode head = new ListNode();

        ListNode node = head;
        head.val = in[in.length - 1];
 ...
 */
//This solution solves in O(n^2) time, but is not accepted by LEETCODE BOIII. But it works and is good enough for an interview.
class ThreeSum {
    ThreeSum() {
        ...
        for(int i = 0; i < nums.length; i++){
            for(int j = 0; j < nums.length; j++){
                if(Start != i && Start != End && i != End){
                    if(nums[Start] + nums[End]+ nums[i] == 0){ //Verify if they equal 0

                        Integer[] TransitionArray = {nums[Start] ,nums[End],nums[i]}; //Do not add if it contains duplicates
                        Arrays.sort(TransitionArray);
                        if(!solSet.contains(Arrays.asList(TransitionArray))) {
                            solSet.add(Arrays.asList(TransitionArray));
                        }
                    }
                }
...
class TwoSum{
    TwoSum(){
        ...
        //To Start nothing is in the list, The first value is always added.
        //Then it checks, does there exist a value such that nums[i] - target is in the list? if so those to numbers sum to target. return indices.
        //This will run in O(n) time.
        for(int i = 0; i < nums.length; i++){

            if(values.contains(target - nums[i])){
                sol[0] = values.indexOf(target - nums[i]);
                sol[1] = i;
                break;
            }
            else{
        ...
        System.out.println("The Two indices are: [" +  sol[0] + "," + sol[1] + "]");
    }
}
